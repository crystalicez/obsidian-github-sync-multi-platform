...
export async function trashVaultFileIfExists(vault: Vault, fileManager: FileManager, path: string): Promise<void> {
  const existing = vault.getAbstractFileByPath(path);
  if (existing instanceof TFile) await fileManager.trashFile(existing);
}
