...
class MemoryGitHub {
  ...
  async createGitBlob(bytes: Uint8Array) { this.blobWrites++; const sha = `blob-${this.blobs.size + 1}`; this.blobs.set(sha, new Uint8Array(bytes)); return sha; }
...
test("remote HEAD change while resolver waits is detected immediately after Continue", async () => {
  ...
  const writesBefore = fixture.github.blobWrites;
  ...
  fixture.github.ref = { ref: "refs/heads/main", sha: "concurrent-head", type: "commit" };
  release.resolve(useLocal(request));

  await assert.rejects(syncing, /conflict.*replan|remote.*head.*changed/iu);
  assert.equal(fixture.github.blobWrites, writesBefore, "stale conflict must stop before upload work");
  assert.equal(fixture.github.refUpdates, updatesBefore, "stale conflict must not update the branch ref");
});
