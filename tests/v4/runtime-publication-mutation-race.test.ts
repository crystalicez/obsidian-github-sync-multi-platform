...
class MemoryGitHub {
  ...
  updateCalls = 0
  beforeNextUpdate?: () => void | Promise<void>
...
function fixture() {
  ...
  const plugin = {
    settings: {
      ...
      vault: "runtime-mutation-race-device",
      consoleLoggingEnabled: false,
    },
    manifest: { id: "runtime-mutation-race-test" },
    app: {
      vault,
      fileManager: {
...
test("runtime retries after an actual updateGitRef publication race and succeeds on attempt two", async () => {
  const { plugin, githubClient, vault } = fixture()
  vault.set("note.md", enc("base\n"), 1)
  ...
  const baseHead = githubClient.ref?.sha
  assert.ok(baseHead)

  vault.set("note.md", enc("local-change\n"), 2)
  githubClient.beforeNextUpdate = async () => {
    const expected = githubClient.ref?.sha
    assert.equal(expected, baseHead)
    const base = githubClient.commits.get(baseHead)
    assert.ok(base)
    const winnerSha = "winner-commit"
    githubClient.commits.set(winnerSha, {
      treeSha: base.treeSha,
      parents: [baseHead],
      message: "external winner",
    })
    githubClient.ref = { ref: "refs/heads/main", sha: winnerSha, type: "commit" }
    githubClient.files = new Map(githubClient.trees.get(base.treeSha))
  }
  ...
  assert.ok(githubClient.updateCalls >= 2, "first CAS must race and a later publication mutation must succeed")
  assert.deepEqual(githubClient.files.get("note.md"), enc("local-change\n"))
  assert.notEqual(githubClient.ref?.sha, "winner-commit")
  runtime.dispose()
})
